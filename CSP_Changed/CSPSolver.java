...
//        variable.domainList.sort(Collections.reverseOrder());
    ...
    private static ArrayList<Variables> backtrackingSolveWithVAH4(ArrayList<Variables> variables) {
        ...
        Variables variable = selectUnassignedVariableVAH4_2(variables);
        if (variable == null)
            return null;
//        variable.domainList.sort(Collections.reverseOrder());
//        Collections.shuffle(variable.domainList);
...
//        Collections.shuffle(variables);
    ...
    private static Variables selectUnassignedVariableVAH4(ArrayList<Variables> variables) {
        //Find the variable that minimizes the ratio of the size of its domain to the number of unassigned constraints
        double min = Double.MAX_VALUE;
        Variables variableWithSmallestRatio = null;
        for (int i = 0; i < variables.size(); i++) {
            if (variables.get(i).domainList.size() != 0 && variables.get(i).value == 0) {
                double unassignedCounts = getUnassignedVariablesCount(variables.get(i));
                double domainSize = variables.get(i).domainList.size();
                //Check for infinity
                if (unassignedCounts == 0) {
                    unassignedCounts = 1;
                }
                double ratio = domainSize / unassignedCounts;
                if (ratio < min) {
                    min = ratio;
                    variableWithSmallestRatio = variables.get(i);
                }
            }
        }
        return variableWithSmallestRatio;
    }

    private static Variables selectUnassignedVariableVAH4_2(ArrayList<Variables> variables) {
